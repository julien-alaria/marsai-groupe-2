...
export default function NavWeb() {
  ...
  return (
    <div className={`hidden md:flex mt-0 ml-2.5 mr-2.5 bg-black/30 text-white text-xl uppercase rounded-full justify-between items-center p-1 border-white/30 border-2 fixed top-8 left-0 right-0 z-50 transition-all duration-300
      ...
      <Link to="/">
        <div className="ml-3 flex gap-2.5 items-center">
          ...
          <div className="text-3xl font-bold uppercase bg-linear-to-b from-[#AD46FF] to-[#F6339A] bg-clip-text text-transparent">AI</div>
        </div>
      </Link>
      ...
      <div className="flex gap-6 items-center font-bold">
        ...
        <Link to="/juryPublic" className="mr-4 hover:text-[#F6339A]">
          <svg height="30" width="30" viewBox="0 0 17 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M10.953 14H6.016v1.031H5V16h7v-.969h-1.047V14zM8.016 4v8.953h.953V4H13.5L8.969 2.833V1.016h-.953v1.817L3.469 4h4.547zM3.492 5.979l1.9 4.005l.566-.207-2.225-4.609c-.053-.098-.157-.16-.298-.152-.128.005-.239.075-.279.175L1.012 9.8l.588.162 1.892-3.983zm9.69-.784-2.158 4.619.592.162 1.902-3.99L15.43 10l.57-.208-2.238-4.619c-.053-.097-.16-.159-.299-.151-.13.003-.24.075-.281.173zm2.802 5.836c0 1.061-1.112 1.922-2.484 1.922-1.372 0-2.484-.861-2.484-1.922h4.968zm-10 0c0 1.061-1.112 1.922-2.484 1.922-1.372 0-2.484-.861-2.484-1.922h4.968z" fill="currentColor" fillRule="evenodd"/>
          </svg>
        </Link>
      ...
      <div className="flex items-center gap-2">
        {/* FR */}
        <button onClick={() => { i18n.changeLanguage('fr'); localStorage.setItem('lang','fr'); }}
                title="Français"
                style={{ opacity: i18n.language === 'fr' ? 1 : 0.5, cursor: 'pointer', background: 'none', border: 'none', transition:'opacity 0.3s'}}>
        ...
        <button onClick={() => { i18n.changeLanguage('en'); localStorage.setItem('lang','en'); }}
                title="English"
                style={{ opacity: i18n.language === 'en' ? 1 : 0.5, cursor: 'pointer', background: 'none', border: 'none', transition:'opacity 0.3s'}}>
          <svg height="25" width="25" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
