...
export default function NavMobile() {
  ...
  return (
    <>
      {/* NAVBAR */}
      <div className={`fixed top-4 left-4 right-4 z-[1000]  h-16 transition-all duration-300
        ${isVisible ? "opacity-70 translate-y-0" : "opacity-0 -translate-y-10"}
        ${isAtTop ? "bg-transparent" : "bg-black/70 backdrop-blur-md"}
        rounded-full px-4 py-3 border border-white/30 flex justify-between items-center md:hidden`}>

        <Link to="/" className="flex items-center gap-2">
          <div className="text-white text-2xl uppercase font-bold">Mars</div>
          <div className="text-2xl font-bold uppercase bg-linear-to-b from-[#AD46FF] to-[#F6339A] bg-clip-text text-transparent">AI</div>
        </Link>
      ...
      {/* OVERLAY FULLSCREEN */}
      {menuOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[2000] flex flex-col justify-start items-center p-6 pt-24 gap-6">

          {/* CLOSE BUTTON */}
          <button
            onClick={() => setMenuOpen(false)}
            className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center
                       text-white text-3xl rounded-full border border-white/30
                       hover:border-[#F6339A] hover:text-[#F6339A] transition"
          >
            ×
          </button>

          {/* LINKS */}
          <Link to="/" className="text-xl hover:text-[#F6339A]">Home</Link>
          <Link to="/program" className="text-xl hover:text-[#F6339A]">Agenda</Link>
          <Link to="/juryPublic" className="text-xl hover:text-[#F6339A]">Jury</Link>
          <Link to="/sponsors" className="text-xl hover:text-[#F6339A]">Sponsors</Link>
          <Link to="/infos" className="text-xl hover:text-[#F6339A]">Infos</Link>
          ...
          <div className="mt-6">
            {firstName ? (
              <>
                <span className="mr-4 text-white">Hello, {firstName}{lastName && ` ${lastName}`}</span>
