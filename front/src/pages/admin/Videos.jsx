...
export default function Videos() {
  ...
  function BatchActions() {
    ...
    /* Finalistes → Publier candidats : active la phase 2 publique sans toucher aux statuts */
    if (activeTab === "finalist") {
      return (
        <div className="flex items-center gap-2 bg-orange-500/10 border border-orange-500/20 rounded-xl px-3.5 py-2 text-[10px] backdrop-blur-sm">
          <span className="text-white/35">{count} finaliste{count > 1 ? "s" : ""} sélectionné{count > 1 ? "s" : ""}</span>
          ...
          >
            {phase2Active ? "⭐ Candidats déjà publiés ✓" : "⭐ Publier candidats"}
          </button>
          <span className="h-3 w-px bg-white/10" />
          <button
            onClick={() => window.confirm(`Refuser ${count} film(s) ?`) && batchStatus("refused")}
    ...
    /* Primés → Publier palmarès (active phase 3 publication) */
    if (activeTab === "awarded") {
      const effectiveActive = phase3Active && tabCounts.awarded > 0;
      return (
        <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-3.5 py-2 text-[10px] backdrop-blur-sm">
          <span className="text-white/35">{count} primé{count > 1 ? "s" : ""} sélectionné{count > 1 ? "s" : ""}</span>
          <span className="h-3 w-px bg-white/10" />
          <button
            onClick={async () => {
          ...
          >
            {effectiveActive ? "🏆 Palmarès déjà publié ✓" : "🏆 Publier palmarès"}
          </button>
          <span className="h-3 w-px bg-white/10" />
          <button onClick={() => setSelectedIds([])} className="text-white/25 hover:text-white/50 transition-all duration-300">✕</button>
        </div>
    ...
    /* Onglets soumis / Phase 1 → actions standard */
    if (activeTab === "submitted" || activeTab === "all") {
      return (
        <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3.5 py-2 text-[10px] backdrop-blur-sm">
          <span className="text-white/35">{count} film{count > 1 ? "s" : ""}</span>
          <span className="h-3 w-px bg-white/10" />
    ...
    /* Autres onglets → actions génériques */
    return (
      <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3.5 py-2 text-[10px] backdrop-blur-sm">
        <span className="text-white/35">{count} film{count > 1 ? "s" : ""}</span>
        <span className="h-3 w-px bg-white/10" />
    ...
    <div className="min-h-screen bg-[#06080d] text-white pb-20">
      ...
      <div className="sticky top-0 z-20 bg-[#06080d]/97 backdrop-blur-xl border-b border-white/5 px-6 py-0">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Tabs + badges de statut de publication */}
          <div className="flex items-center justify-between border-t border-white/5">
            ...
            {(tabCounts.finalist > 0 || tabCounts.awarded > 0) && (
              <div className="flex items-center gap-2 px-4">
                {tabCounts.finalist > 0 && (
                  <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-medium border ${
                    phase2Active
                      ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"
                      : "bg-white/5 border-white/10 text-white/25"
                  }`}>
                    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${phase2Active ? "bg-emerald-400 animate-pulse" : "bg-white/15"}`} />
                    {phase2Active ? "Candidats publiés" : "Candidats non publiés"}
                  </span>
                )}
                {tabCounts.awarded > 0 && (
                  <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-medium border ${
                    (phase3Active && tabCounts.awarded > 0)
                      ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-400"
                      : "bg-white/5 border-white/10 text-white/25"
                  }`}>
                    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${(phase3Active && tabCounts.awarded > 0) ? "bg-yellow-400 animate-pulse" : "bg-white/15"}`} />
                    {(phase3Active && tabCounts.awarded > 0) ? "Palmarès publié" : "Palmarès non publié"}
                  </span>
                )}
              </div>
            )}
      ...
      <div className="max-w-7xl mx-auto px-6 pt-5 space-y-4">
        ...
        {/* Bannière contextuelle pour Finalistes et Primés */}
        {activeTab === "finalist" && filteredMovies.length > 0 && (
          <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-orange-500/8 border border-orange-500/20 backdrop-blur-sm">
            <div className="flex items-center gap-2.5">
              <span className="w-2 h-2 rounded-full bg-orange-400 flex-shrink-0" />
              <p className="text-[11px] text-orange-200/70 leading-relaxed">
                <span className="font-medium text-orange-300">Finalistes</span> — Sélectionnez des films puis cliquez sur <span className="font-semibold text-orange-300">⭐ Publier candidats</span> pour les rendre publics.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setSelectedIds(filteredMovies.map((m) => m.id_movie))}
              className="flex-shrink-0 text-[10px] px-3 py-1.5 rounded-lg bg-orange-500/15 border border-orange-500/25 text-orange-300 hover:bg-orange-500/25 transition-all duration-300 font-medium whitespace-nowrap"
            >
              Tout sélectionner
            </button>
          </div>
        ...
        {activeTab === "awarded" && filteredMovies.length > 0 && (() => {
          const effectiveActive = phase3Active && tabCounts.awarded > 0;
          return (
            <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-yellow-500/8 border border-yellow-500/20 backdrop-blur-sm">
              <div className="flex items-center gap-2.5">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${effectiveActive ? "bg-yellow-400 animate-pulse" : "bg-yellow-600"}`} />
                <p className="text-[11px] text-yellow-200/70 leading-relaxed">
                  {effectiveActive
                    ? <><span className="font-medium text-yellow-300">🏆 Palmarès publié</span> — Le palmarès est actuellement visible sur le site public.</>
                    : <><span className="font-medium text-yellow-300">Primés</span> — Sélectionnez des films puis cliquez sur <span className="font-semibold text-yellow-300">🏆 Publier palmarès</span> pour publier.</>
                  }
                </p>
              </div>
              <button
                type="button"
                onClick={() => setSelectedIds(filteredMovies.map((m) => m.id_movie))}
                className="flex-shrink-0 text-[10px] px-3 py-1.5 rounded-lg bg-yellow-500/15 border border-yellow-500/25 text-yellow-300 hover:bg-yellow-500/25 transition-all duration-300 font-medium whitespace-nowrap"
              >
                Tout sélectionner
              </button>
            </div>
