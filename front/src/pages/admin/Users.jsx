  ...
  <div className="min-h-screen bg-gradient-to-br from-[#0a0c0f] to-[#0d0f12] text-white pt-6 pb-12 px-4">
    ...
    <section className="bg-gradient-to-br from-[#1a1c20]/60 to-[#0f1114]/60 backdrop-blur-xl border border-white/10 rounded-xl p-3 sm:p-4 shadow-xl shadow-black/30 transition-all duration-300">
      ...
      {message && (
        <div className={`mb-3 p-2 sm:p-3 rounded-lg ${
          ...
          <div className="flex items-center text-xs sm:text-sm">
            {message.includes("succès") ? (
              <svg className="w-3 h-3 sm:w-4 sm:h-4 mr-2 text-green-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
              </svg>
            ) : (
              <svg className="w-3 h-3 sm:w-4 sm:h-4 mr-2 text-red-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <div>
          <h2 className="text-base sm:text-lg font-semibold text-white">Gestion des utilisateurs</h2>
          <p className="text-[10px] sm:text-xs text-gray-400 mt-1">
            {searchQuery
              ? `${displayInfo.total} résultat${displayInfo.total !== 1 ? "s" : ""} pour "${searchQuery}"`
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          ...
          <button 
            ...
            className="px-3 py-1.5 text-xs sm:text-sm bg-gradient-to-r from-blue-600/30 to-blue-700/30 text-blue-200 border border-blue-500/30 rounded-lg hover:from-blue-700/40 hover:to-blue-800/40 hover:text-white hover:scale-[1.02] transition-all duration-200 shadow hover:shadow-blue-500/20 whitespace-nowrap"
          >
            + Nouvel utilisateur
          </button>
        </div>
  ...
  <div className="overflow-x-auto">
    <table className="min-w-full">
      <thead className="bg-gray-900/70">
        <tr>
          <th className="px-2 sm:px-3 py-1.5 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase border-b border-white/10">Prénom</th>
          <th className="px-2 sm:px-3 py-1.5 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase border-b border-white/10">Nom</th>
          <th className="px-2 sm:px-3 py-1.5 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase border-b border-white/10 hidden sm:table-cell">Email</th>
          <th className="px-2 sm:px-3 py-1.5 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase border-b border-white/10">Rôle</th>
          <th className="px-2 sm:px-3 py-1.5 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase border-b border-white/10 w-16 sm:w-20">Actions</th>
        </tr>
      </thead>
        ...
        {paginatedUsers.length > 0 ? (
          paginatedUsers.map((user) => (
            <tr key={user.id_user} className="group hover:bg-white/5 transition-colors duration-150">
              <td className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm text-gray-200 border-b border-white/5 truncate max-w-[80px] sm:max-w-none">{user.first_name}</td>
              <td className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm text-gray-200 border-b border-white/5 truncate max-w-[80px] sm:max-w-none">{user.last_name}</td>
              <td className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm text-gray-300 truncate max-w-[120px] border-b border-white/5 hidden sm:table-cell" title={user.email}>
                {user.email}
              </td>
              <td className="px-2 sm:px-3 py-1.5 border-b border-white/5">
                <span className={`px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[10px] font-medium rounded-full ${
          ...
          <tr>
            <td colSpan="5" className="px-3 sm:px-6 py-6 text-center text-gray-400 text-xs sm:text-sm">
              {searchQuery
...
{showCreateModal && (
  <div className="fixed inset-0 bg-black/60 backdrop-blur flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
    <div className="bg-gradient-to-br from-gray-900/90 to-gray-950/90 border border-white/10 rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl shadow-black/40 max-h-[90vh] overflow-y-auto">
      {/* En-tête minimaliste */}
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        <h3 className="text-base sm:text-lg font-semibold text-white">Nouvel utilisateur</h3>
        <button
          onClick={() => {
            setShowCreateModal(false);
            createForm.reset();
      ...
      <form onSubmit={createForm.handleSubmit(onCreateSubmit)} className="space-y-3 sm:space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {/* Champ: Prénom */}
          <div>
            <label className="block text-[10px] sm:text-xs font-medium mb-1 text-gray-400">Prénom</label>
            <input 
              type="text" 
              {...createForm.register("firstName")} 
              className={`w-full bg-black/30 border rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-white 
            ...
            />
            {createForm.formState.errors.firstName && (
              <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{createForm.formState.errors.firstName.message}</p>
            )}
          </div>

          {/* Champ: Nom */}
          <div>
            <label className="block text-[10px] sm:text-xs font-medium mb-1 text-gray-400">Nom</label>
            <input 
              type="text" 
            ...
            />
            {createForm.formState.errors.lastName && (
              <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{createForm.formState.errors.lastName.message}</p>
            )}
          </div>
        </div>
        ...
        {/* Champ: Email */}
        <div>
          <label className="block text-[10px] sm:text-xs font-medium mb-1 text-gray-400">Email</label>
          ...
          {createForm.formState.errors.email && (
            <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{createForm.formState.errors.email.message}</p>
          )}
        </div>

        {/* Champ: Mot de passe */}
        <div>
          <label className="block text-[10px] sm:text-xs font-medium mb-1 text-gray-400">Mot de passe</label>
          <div className="relative">
            <input 
              type="password" 
          ...
          {createForm.formState.errors.password && (
            <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{createForm.formState.errors.password.message}</p>
          )}
        </div>

        {/* Champ: Rôle - style compact */}
        <div>
          <label className="block text-[10px] sm:text-xs font-medium mb-1 text-gray-400">Rôle</label>
          <div className="relative">
            <select 
              {...createForm.register("role")} 
              className="w-full bg-black/30 border border-white/10 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-white 
                       focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all duration-200 
                       appearance-none cursor-pointer"
            >
              <option value="PRODUCER" className="bg-gray-900 text-xs sm:text-sm">Producteur</option>
              <option value="JURY" className="bg-gray-900 text-xs sm:text-sm">Jury</option>
              <option value="ADMIN" className="bg-gray-900 text-xs sm:text-sm">Administrateur</option>
            </select>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            <div className={`px-1.5 sm:px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-medium border ${
        ...
        <div className="flex justify-end gap-2 pt-3 sm:pt-4 border-t border-white/5">
          <button 
            type="button"
            onClick={() => {
              setShowCreateModal(false);
            ...
            className="px-3 py-1.5 text-[10px] sm:text-xs border border-white/10 rounded hover:bg-white/5 text-gray-400 hover:text-white transition-all duration-200"
          >
            Annuler
          </button>
          <button 
            type="submit"
            className="px-3 sm:px-4 py-1.5 bg-gradient-to-r from-blue-600/80 to-blue-700/80 text-white text-[10px] sm:text-xs rounded 
...
{showEditModal && editingUser && (
  <div className="fixed inset-0 bg-black/60 backdrop-blur flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
    <div className="bg-gradient-to-br from-gray-900/90 to-gray-950/90 border border-white/10 rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl shadow-black/40 max-h-[90vh] overflow-y-auto">
      ...
      <div className="flex justify-between items-center mb-4 sm:mb-6">
        <div>
          <h3 className="text-base sm:text-lg font-semibold text-white">Modifier l'utilisateur</h3>
      ...
      <form onSubmit={editForm.handleSubmit(onUpdateSubmit)} className="space-y-3 sm:space-y-4">
        {/* Grille compacte Prénom/Nom */}
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {/* Champ: Prénom */}
          <div>
            ...
            />
            {editForm.formState.errors.firstName && (
              <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{editForm.formState.errors.firstName.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {editForm.formState.errors.lastName && (
              <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{editForm.formState.errors.lastName.message}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          {editForm.formState.errors.email && (
            <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{editForm.formState.errors.email.message}</p>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="block text-[10px] sm:text-xs font-medium text-gray-400">Mot de passe</label>
            <span className="text-[8px] sm:text-[10px] text-gray-500">Optionnel</span>
          </div>
          <div className="relative">
            <input 
              type="password" 
          ...
          <p className="text-gray-500 text-[8px] sm:text-[10px] mt-1">Laisser vide pour conserver l'actuel</p>
          {editForm.formState.errors.password && (
            <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">{editForm.formState.errors.password.message}</p>
          )}
        </div>
      ...
      {showDeleteConfirm && userToDelete && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 rounded-xl p-4 sm:p-6 w-full max-w-md shadow-2xl shadow-black/50">
            <div className="text-center">
              ...
              <h3 className="text-base sm:text-lg font-semibold text-white mb-2">Confirmer la suppression</h3>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-3">
                <button
                  ...
                  className="px-4 sm:px-5 py-2 border border-white/10 rounded-lg hover:bg-white/10 text-gray-300 hover:text-white transition-all duration-300 text-xs sm:text-sm"
                ...
                <button
                  ...
                  className="px-4 sm:px-5 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg hover:from-red-700 hover:to-red-800 transition-all duration-300 shadow-lg hover:shadow-red-500/30 flex items-center justify-center text-xs sm:text-sm"
                  disabled={deleteMutation.isLoading}
                >
